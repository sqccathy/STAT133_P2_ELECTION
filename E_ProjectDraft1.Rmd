...
if(createSource2) {
  ...
  # I checked where the NA is, and if the min 8 is reasonable.
  ...
  # There is one row of irrelevant information in Mississipi. Thus I dropped it from the data frame.
  ...
  # The min occurs in king, texas. According to Wikipedia, as of the 2010 census, its population was 286, making it the second-least populous county in Texas and the third-least populous of any county in the United States. Thus the minimum being 8 makes sense.
...
if(createSource6) {
  ...
  # SPLIT STATE NAMES AND COUNTY NAMES
  censusData$GEO.display.label = as.character(censusData$GEO.display.label)
  names = strsplit(as.character(censusData$GEO.display.label), split = ", ")
  
  # STATE NAMES
  sNames = sapply(names, function(x) x[2])
  
  # APPLY REGULAR EXPRESSION AND STRING MANIPULATION TO STATE NAMES
  sNames = gsub(" ", "", sNames)
  sNames = tolower(sNames)
  ...
  # COUNTY NAMES
  cNames = sapply(cNames, function(x) x[1])
  
  # APPLY REGULAR EXPRESSION AND STRING MANIPULATION TO COUNTY NAMES
  cNames = gsub("&", "and", cNames)
  cNames = gsub("-", "", cNames)
  cNames = gsub(" ", "", cNames)
  cNames = tolower(cNames)

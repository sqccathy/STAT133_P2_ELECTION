...
num_of_row = sapply(list,nrow)
snames = rep(snames,num_of_row)
cnames = unlist(sapply(list,function(x) x[[1]]))
Obama = unlist(sapply(list,function(x) x[[4]]))
McCain = unlist(sapply(list,function(x) x[[5]]))
countyVotes2008 = data.frame(State=snames,County = cnames,
                             obamaVote2008=Obama,McCainVote2008 = McCain)
...
#add a column to merge
snames = tolower(gsub(" ","",countyVotes2008$State))
countyVotes2008$County = paste(tolower(countyVotes2008$County),",",snames,sep="")
```

## Election Results in 2012
```{r}
# Code by Jieni Wan
# Convert stateNames to a vector.
...
library(XML)
# Create a vector consisting of all the websites we are going to use by pasting.
xml2012 = paste0("http://www.stat.berkeley.edu/users/nolan/data/voteProject/countyVotes2012/", stateNames, ".xml")
# For each website, use xmlParse function to read the xml file and then get a list.
xmlList = lapply(xml2012, xmlParse)
# For each xml file, extract the information we want, i.e. the votes for Obama and Romney and also the county names.
...
# Convert the lists obtained to vectors.
ObamaVote2012 = unlist(Obama)
ObamaVote2012 = gsub(" ", "", ObamaVote2012)
ObamaVote2012 = as.numeric(gsub(",", "", ObamaVote2012))

RomneyVote2012 = unlist(Romney)
RomneyVote2012 = gsub(" ", "", RomneyVote2012)
RomneyVote2012 = as.numeric(gsub(",", "", RomneyVote2012))

cNames = unlist(cNames)
# Revise the format of the county names.
cNames = tolower(cNames)
...
# Paste the county names with the state names corresponding to the counties. 
cNames = paste0(cNames, ",", rep(stateNames, sapply(Obama, length)))
# Create a data frame with three columns, County, ObamaVote2012 and RomneyVote2012
countyVotes2012 = data.frame(ObamaVote2012, RomneyVote2012)
...
#State names
sNames = as.character(xpathSApply(doc, "/doc/state/gml:name", xmlValue))

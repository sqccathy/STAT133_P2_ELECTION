...
For convenience in final merging, apply regular expressions and string manipulations to county names and state names so that they have the same format. Stipulate that all names should only contain lowercase letters. Then add the state names and county names we get to the data frame, countyVotes2004, as new variables. Subset the data frame to extract the information we want, namely, county names, state names, number of votes for Bush and Kerry.
```{r eval = creat2004}
...
Notice that data of votes in Virginia are missing. So scrape those data from HTML tables in Wikipedia and add them to the data frame, countyVotes2004. Of course, names of counties and states should be converted to the same format mentioned above.  
```{r}
  ...
  # County names
  cNames = c(counties, independentCity)
...
Carry out EDA (Exploratory Data Analysis) to check whether the data are clean or not by referring to summary statistics of the numeber of votes for the two candidates. It turns out that the minimum of votes for Bush is only 65 and the maximum is 1670341, which may not be reasonable. Therefore we find the two rows corresponding to the min and max votes respectively to see the specific locations where the min and max occur. 

The min occurs in Loving, Texas. According to Wikipedia, as of the 2010 census, its population was 82, making it the least populous county in the United States. In addition, The max occurs in Los Angeles, California and the 2010 United States Census reported that Los Angeles had a population of 3,792,621. Thus the minimum being 54 and the maximum being 954764 make sense.  
```{r}
...
Similarly, use summary function for the number of votes for Kerry and check whether the minimum and maximum are reasonable. Actually, th places where the min and max occur are again Loving in Texas and Los Angeles in California respectively, which account for the numbers of votes.
```{r eval = create2004} 
...
```{r eval = create2012}
 ...
 Carry out EDA. Use summary statistics of the number of votes for Obama and Romney. It turns out that no NA exists and the values of these statistics of votes for Romney are acceptable, but the minimum of votes for Obama is only 5, which may be unreasonable. To check this, we need to find the row in which the number of votes for Obama is 5. And it turns out that the min occurs in King, Texas. It is acceptable due to previous explanation. 
 ```{r eval = create2012}
  # Summary statistics of number of votes for Obama
...
```{r eval = createCensus}
  ...
  cNames = gsub("\\.", "", cNames)
  cNames = tolower(cNames)
  cNames = gsub("county|parish|city|municipality|municipio|censusarea", "", cNames)
  cNames = gsub("&", "and", cNames)
...
merge0812[which(is.na(merge0812$ObamaVote2012)), ]
#         County  State ObamaVote2012 RomneyVote2012 obamaVote2008 McCainVote2008 
#  3114   alaska alaska       NA             NA         80505         136585
merge0812[which(is.na(merge0812$obamaVote2008)), ]
#       County          State           ObamaVote2012 RomneyVote2012 obamaVote2008 McCainVote2008
#784 districtofcolumbia districtofcolumbia  222332        17337            NA           NA
```

...
```
  ...
  It turns out that no NA exists and the values of these statistics of votes for Romney are acceptable, but the minimum of votes for Obama is only 5, which may be unreasonable. To check this, we need to find the row in which the number of votes for Obama is 5.
```{r eval = create2012}  
...
```
  ...
  The minimum number of votes for Clinton is 4, which is unusually small. Check if it is reasonable.
```{r eval = create2016}  
...
```{r eval = createCensus}
  ...
  # STATE & COUNTY NAMES
  cNames = cNamesManip(as.character(censusData$GEO.display.label))
  ...
  censusData$County = sapply(names, function(x) x[1])

  # ADD STATE NAMES TO THE DATA FRAME
    
  censusData$State = sapply(names, function(x) x[2])

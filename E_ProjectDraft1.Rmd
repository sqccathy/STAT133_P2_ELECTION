...
if(createSource2) {
  ...
  # Check if there is any county belonging to Hawaii or Alaska. If so, drop these counties
  if(any(countyVotes2008$State == "hawaii" | countyVotes2008$State == "alaska")) {
    countyVotes2008 = countyVotes2008[countyVotes2008$State != "hawaii" &
                                      countyVotes2008$State != "alaska", ]
  }
  ...
  # EDA of number of votes for Obama
  ...
  # The min occurs in King County, Texas. According to Wikipedia, as of the 2010 census, its population was 286, making it the second-least populous county in Texas and the third-least populous of any county in the United States. Thus the minimum being 8 makes sense.
  
  # Check if the maximum is reasonable
  countyVotes2008[which(countyVotes2008$obamaVote2008 == max(countyVotes2008$obamaVote2008)), ]
  #        County      State obamaVote2008 McCainVote2008
  #177 losangeles california       1938744         826512
  
  # The max occurs in Los Angeles, California. According to Wikipedia, the 2010 United States Census reported that Los Angeles had a population of 3,792,621. Thus the maximum being 1938744 makes sense.
...
if(createSource3) {
  # Get state names
  sNames = read.csv("http://www.stat.berkeley.edu/users/nolan/data/voteProject/countyVotes2012/stateNames.txt")
  sNames = as.vector(sNames$states)
  
  # Drop hawaii and alaska from stateNames since we have decided to skip them
  sNames = sNames[sNames != "hawaii" & sNames != "alaska"]
  ...
  # NA does not exist, but the minimum is 5 and the maximum is 1672000, which may be unreasonable.
  
  # Check the minimum and the maximum to see if they are acceptable.
  countyVotes2012[which(countyVotes2012$ObamaVote2012 == 5), ]
  
  #        ObamaVote2012   RomneyVote2012     County
  # 2628          5            139          king,texas
  
  countyVotes2012[which(countyVotes2012$ObamaVote2012 == max(countyVotes2012$ObamaVote2012)), ]
  
  #        ObamaVote2012   RomneyVote2012        County
  # 176       1672164         699600        losangeles,california 
  
  # The min occurs in king, texas and the max occurs in losangeles, california, which are acceptable due to the similar reasons mentioned above. 
  ...
  # Only need to check the minimum.
  countyVotes2012[which(countyVotes2012$RomneyVote2012 == 54), ]
  
  #        ObamaVote2012   RomneyVote2012    County
  #  2644         9             54          loving,texas
  
  # The min occurs in loving, texas. According to Wikipedia, as of the 2010 census, its population was 82, making it the least populous county in Texas. Thus the minimum being 54 makes sense.
```
...
if(createSource4) {
  ...
  # Drop Alaska and Hawaii from the data frame
  countyVotes2016 = countyVotes2016[countyVotes2016$state_abbr != "AK" &
                                    countyVotes2016$state_abbr != "HI", ]
  ...
  # We keep the FIPS, in order to merge with other data sources; we also keep numbers of votes for Clinton and Trump
  countyVotes2016 = countyVotes2016[, c(11, 2, 3)]
...
if(createSource5) {
  ...
  # Since we have decided that we won't look into Hawaii or Alaska, we will have to make sure that we drop the counties in Hawaii or Alaska. We will first check if there is any county belonging to Hawaii or Alaska.
  if(any(GML_df$State == "hawaii" | GML_df$State == "alaska")) {
    GML_df = GML_df[GML_df$State != "hawaii" & GML_df$State != "alaska", ]
  }
...
```
  ...
  According to the plot above, it is safe to say that the map looks right, which means we have properly cleaned the data.
...
if(createSource6) {
  ...
  # Drop the counties in Alaska or Hawaii.
  censusData = censusData[censusData$State != "alaska" &
                          censusData$State != "hawaii", ]

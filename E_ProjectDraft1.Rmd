...
For the election results in 2008, we have the excel file. First I use the r package xlsx to read the xlsx file and get the sheet names, which represents the state. Notice the first sheet is the summary of the states results, so I skip that sheet.Then I get a list containing 50 dataframes of the countys' election results for each state. By getting the number of rows in each dataframe in the list, I replicate the states names so that each county has the corresponding state in the final result. After that, I extract the county names, number of Obama votes and Mccain votes from the list. At last I put the above four variables into one dataframe. 
```{r eval = create2008}
library(xlsx)
...
```
  ...
  We decide to remove Alaska data, to keep consistent with other data.
...
The census data required that we merge three files together, one with population and race information, one with data on households and family structure, and a third with employment data. We first extract the count of total population and the count of people identitfying as white. We do not extract data on the count of individuals identifying as black, because there are too many counties missing this data. Using the total population and the count of white indidividuals, we derive the percent of population that is white in each county. 
...
The eda by summary statistics indicates that are three counties that have data on total population, but lack data on the number of white residents. 

We then extract data from the second file on family structures within counties. We look at, e.g., families with children, rates of single parenthood, seniors living alone, rates of married and never married men, and the average size of families in counties.
...
```
  ...
  The summary statistics indicate that 78 counties appear in the population data that are not in the family and household data. These turn out to all be in Puerto Rico. 
...
The exploratory data analysis with summary statistics indicates that there are no missing counties for the employment data. 
...
There are no changes in the summary statistics by variable after merging the family and population data frame with the employment data frame. There are 3,217 counties represented. There are no missing counties for total population, but 3 missing for counts of white population (and by extension the percent of the population that is white). There are still the 78 counties missing family structure data, all from Puerto Rico. There are no missing observations for employment data. 
...
The Guardian. (2016). 2008 Presidential Election Results by State and County. 
  Available from https://www.theguardian.com/news/datablog/2009/mar/02/us-elections-2008

...
cNames$B01003 = as.character(B01003$GEO.display.label)
cNames$B01003 = gsub(" ", "", cNames) 
cNames$B01003 = gsub("&", "and", cNames) 
cNames$B01003 = gsub("county|parish", "", cNames) 
cNames$B01003 = gsub("\\.", "", cNames)
cNames$B01003 = tolower(cNames) 

totalPop$B01003 = B01003$HD01_VD01 #1 Total population

DP02 = read.csv("http://www.stat.berkeley.edu/~nolan/data/voteProject/census2010/DP02.csv")
cNames$DP02 = as.character(DP02$GEO.display.label)
cNames$DP02 = gsub(" ", "", cNames) 
cNames$DP02 = gsub("&", "and", cNames) 
cNames$DP02 = gsub("county|parish", "", cNames) 
cNames$DP02 = gsub("\\.", "", cNames)
cNames$DP02 = tolower(cNames) 

familiesWithKids$DP02 = DP02$HC03_VC06 #2 Percent of households that are families with Children under 18
singleDads$DP02 = DP02$HC03_VC10 #3 Percent households with a single male head of household with children under 18
singleMoms$DP02 = DP02$HC03_VC12 #4 Percent of households with a single female head of household with children under 18
seniorsLivingAlone$DP02 = DP02$HC03_VC15 #5 Housholder living alone, 65 years old or over

DP03 = read.csv("http://www.stat.berkeley.edu/~nolan/data/voteProject/census2010/DP03.csv")
cNames$DP03 = as.character(DP02$GEO.display.label)
cNames$DP03 = gsub(" ", "", cNames) 
cNames$DP03 = gsub("&", "and", cNames) 
cNames$DP03 = gsub("county|parish", "", cNames) 
cNames$DP03 = gsub("\\.", "", cNames)
cNames$DP03 = tolower(cNames)

censusDataFrame = merge(x = B01003, y = DP02, z = DP03, all = TRUE)

#I am going to keep woring on this tomorow morning 
```

---
title: "2016 Presidential Election Debrief Project - Data Wrangling"
output: html_document
author: Menglu Cao, Jieni Wan, Qichen Sun, John Towey, Victor Choi
---
  
  This report focuses on data wrangling. First we acquire and clean presidential election results data from 2004 to 2016, geographic data and census data in 2010, all on county level. Note that these data are of different formats, and are from different sources. Our goal here is to create one comprehensive data frame based on these data. When we merge them together, again, we'll need to perform data cleaning.
  
  The following set up helps us control which code is run. We can create each data frame once and save the data frame in an rda file. Then in the future, we do not recreate this file but load only the rda file.
```{r}
create2004 = FALSE
create2008 = FALSE
create2012 = FALSE
...
# Remove data frames that we don't need any more
rm(Virginia2004)
```
...
# Import xlsx file into a list. Notice the first sheet is the result of the state level, so we ignore it.
...
rm(xmlList, Obama, Romney, ObamaVote2012, RomneyVote2012)
```
...
```
  ...
  With all the variables needed in position, we can then create a data frame, which contains county names, state names, and the latitude and longitude for each county. Then we remove unneeded variables.
```{r eval = createGML} 
GML_df = data.frame(County = cNames, State = sNames, Latitude, Longitude, stringsAsFactors = FALSE)

rm(Longitude, Latitude, num_of_county)
```
...
populationData$percentWhite = 100 * (populationData$whitePop / populationData$totalPop) #3 White population as a percent of total population

# REMOVE UNNEEDED DATA FRAMES

rm(B01003, totalPopFrame, whitePopFrame)
```
...
``` 
  ...
  There are no missing values. Then we merge family data with population data and remove redundant data frames.
```{r eval = createCensus}
...
employmentData$femaleUnemployment = employmentData$femaleLaborForce - employmentData$employedWomen
...
```
  ...
  After we extract the variables, we combine them into one data frame and assign a name for counties and states that matches those of the aformentioned data frames--election results data and GML data. We will then remove unneeded data frames.
```{r eval = createCensus}
...
### Data Organization
  ...
  This part was mutually accomplished by Victor Choi, Jieni Wan, Qichen Sun & Menglu Cao.

## Merging 2008 & 2012 Election Results

  At last, we can merge all of the data into one data frame for further analys
...
```
  ...
  We find that the 2008 data does not contain data for District of Columbia, so we decide to code it in. We also find that 2012 data does not contain data for Alaska, so we decide to remove the row.
```{r eval = merge}
merge0812[merge0812$County == "districtofcolumbia", c(5, 6)] = c(245800, 17367)
merge0812 = merge0812[merge0812$County != "alaska", ]
```

  We should perform the EDA again to make sure the cleaning works as expected.
```{r eval = merge}
merge0812[which(!complete.cases(merge0812)), ]
```
...
## Merging GML & Census Data

  Next, we take the union merge of the GML data and the census data. And we perform EDA to the new data frame.
```{r eval = merge}
mergeGMLCensus = merge(GML_df, censusData, by = c("County", "State"), all = TRUE)
mergeGMLCensus[!complete.cases(mergeGMLCensus), ]
```

  After performing EDA, we decide to take out Alaska since merge0812 doesn't contain counties in Alaska. Also, there is no census data for King, Loving, and Kenedy Texas, South Boston City, and Clifton Forge City, so we take them out as well. GML data for Broomfield, Colorado is missing, so we add it in. To solve the Tilde problem in Donaana, New Mexico, we decide to manually fix it.
  The rest of NAs is from holes only in the Census Data. We take that row out.
```{r eval = merge}
mergeGMLCensus = mergeGMLCensus[(which(mergeGMLCensus$State != "alaska")), ]
...
mergeGMLCensus[mergeGMLCensus$County == "doñaana",  c("County", "Latitude", "Longitude")] = c("donaana", 32.34523, -106.83238)
mergeGMLCensus = mergeGMLCensus[mergeGMLCensus$County != "kalawao", ]
```
...
## Merging 2004 Election Result and GML&Census Data

  Next we merge the 2004 election result data and the GML&Census data together.
```{r eval = merge}
merge04GMLCensus = merge(countyVotes2004, mergeGMLCensus, by = c("County", "State"), all = TRUE)
merge04GMLCensus[!complete.cases(merge04GMLCensus), ]
```

  We find that countyVotes2004 doesn't have data in Hawaii, so we decide to take them out.
  As for King, Loving and Kenedy county at Texas, we do the same manipulation as before.
  We also find that the names of washington and miami-dade are different in these 2 data frames, so we manually fix these.
```{r eval = merge}
...
merge04GMLCensus[merge04GMLCensus$County == "districtofcolumbia", c(3, 4)] = c(19007, 183876)
merge04GMLCensus = merge04GMLCensus[!(merge04GMLCensus$County == "washington" & merge04GMLCensus$State == "districtofcolumbia"), ]

merge04GMLCensus[merge04GMLCensus$County == "miamidade", c(3, 4)] = c(358613, 406099)
merge04GMLCensus = merge04GMLCensus[!(merge04GMLCensus$County == "dade" & merge04GMLCensus$State == "florida"), ]
```
...
## Merging 2016 Election Result and 04&GML&Census Data
  
  Next we merge the above data frame with the 2016 election result data.
```{r eval = merge}
merge04GMLCensus16 = merge(countyVotes2016, merge04GMLCensus, by.x = "combined_fips", by.y = "FIPS", all = TRUE)
merge04GMLCensus16[!complete.cases(merge04GMLCensus16), ]
```

  We notice that all the NAs are results of the fact that the 2016 election result has data in Alaska, while the other data frame does not. So we skip Alaska by taking them out.
```{r eval = merge}
...
## Final Merge
  ...
  Finally, we merge the result for 0812 with 04GMLCensus16 by county and state, and get the rows with NAs into a new data frame.
```{r eval = merge}
...
```
  ...
  To solve the matching problem, we first specify the rows that contain such "xxxcity" names and those with corresponding names, "xxx", in the data frame, NAs. Since "xxx" and "xxxcity" stand for the same place, we just need to keep one of them.  We add missing values to the rows with "xxxcity" names by referring to the values in rows with correspondinbg "xxx" names. 
```{r eval = merge}
city = c(36, 273, 305, 457, 483, 606, 651, 721, 882, 909, 987, 1010, 1179, 1226, 1297, 1656, 1739, 1783, 1785, 1836, 2075, 2097, 2106, 2246, 2320, 2325, 2380, 2501, 2670, 2728, 2913, 3018, 3078, 3087)
county = city - 1
final[city, c(42, 43, 44, 45)] = final[county, c(42, 43, 44, 45)]
```

  In addition, one county can have several different names in US. For example, Brooklyn in New York has another name, Kings. Thus, the row with county name, brooklyn, and state name, newyork, cannot be connected with the row with county name, kings, and state name, newyork by merging. We find out all such cases and conflate information for the same place together. Again, we just need to keep one row for each pair.  
```{r eval = merge}
final[c(1433, 1434, 2695, 2705, 1514, 2082, 2430), c(42, 43, 44, 45)] = final[c(1404, 1405, 2497, 2498, 278, 1787, 2668), c(42, 43, 44, 45)]
```

  After adding the missing values, we perform the EDA again to check if the cleaning works as expected. Turns out, it does. And we find that the rest of NAs are only concerned with the missing data in census data. Considering the difficulty in obtaining those missing data, we evantually decide to drop them and get our final data frame.
```{r eval = merge}
final = final[complete.cases(final), ]
```

  To save storage and make the following analysis easier, we've decided to drop unnecessary columns, and rearrange the variable sequences. 
```{r eval = merge}
final = final[c(1, 2, 10, 11, 8, 9, 44, 45, 42, 43, 6, 7, 12:41)]
save(final, file = "finalDataFrame.rda")
```

  At last, we remove the intermediate data frames.
```{r eval = merge}
rm(merge0812, mergeGMLCensus, merge04GMLCensus, merge04GMLCensus16)
```
...
United States Census Bureau. (2016). American Fact-Finder. Available from       
...
United States presidential election results in Florida, 2004
  https://en.wikipedia.org/wiki/United_States_presidential_election_in_Florida,_2004

United States presidential election results in Hawaii, 2012  
  https://en.wikipedia.org/wiki/United_States_presidential_election_in_Hawaii,_2012#By_county

United States presidential election results in New York, 2012
  https://en.wikipedia.org/wiki/United_States_presidential_election_in_New_York,_2008#By_county
...
United States presidential election in Virginia, 2008
  https://en.wikipedia.org/w/index.php?printable=yes&title=United_States_presidential_election_in_Virginia,_2008
